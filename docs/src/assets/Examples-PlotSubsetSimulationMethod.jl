...
begin
    ...
    contour!(x_1_range, x_2_range, g_samples; levels=[0], color=(:black, 0.25))
    ...
    for i in eachindex(solution.CSubset)
        ...
        scatter!(
            solution.XSamplesSubset[i][1, 1:100:end],
            solution.XSamplesSubset[i][2, 1:100:end];
            alpha=0.5,
    ...
    tooltip!(1.0, 1.0, L"$\Omega_{f_{1}}$"; offset=0, outline_linewidth=1, fontsize=12)

    tooltip!(1.7, 1.7, L"$\Omega_{f_{2}}$"; offset=0, outline_linewidth=1, fontsize=12)

    tooltip!(2.4, 2.4, L"$\Omega_{f_{3}}$"; offset=0, outline_linewidth=1, fontsize=12)
