...
function randomvariable(
    ...
    by::AbstractString,
    values::Union{Real,AbstractVector{<:Real}},
)
    ...
    # Error-catching:
    distribution in supp_distributions ||
        throw(ArgumentError("Provided distribution is not supported!"))
    (by == "M" || by == "P") || throw(
        ArgumentError(
            """Random variables can only be defined by "moments" ("M") and "params ("P")"!""",
        ),
    )
    ...
    random_variables = getfield((@__MODULE__).Distributions, Symbol(distribution))(
        values...
    )
...
function convertmoments(
    distribution::AbstractString, moments::Union{Real,AbstractVector{<:Real}}
)
    # Error-catching:
    length(moments) == 2 || throw(
        ArgumentError(
            "Too few or many moments are provided! Provide only the mean (μ) and standard deviation (σ) in a vector format (i.e., [μ, σ])!",
        ),
    )
    ...
    if distribution == "Exponential"
        ...
        if mean != std
            throw(
                DomainError(
                    moments, "mean and standard deviation values of must be the same!"
                ),
            )
        end
    ...
    if distribution == "Frechet"
        ...
        FFrechet(u, p) =
            sqrt(
                SpecialFunctions.gamma(1 - 2 / u) - SpecialFunctions.gamma(1 - 1 / u) ^ 2
            ) / SpecialFunctions.gamma(1 - 1 / u) - p
        u_0 = (2 + 1E-1, 1E+6)
        p_0 = std / mean
        ...
        if !isapprox(FFrechet(α, p_0), 0; atol=1E-9)
            throw(
                DomainError(
                    moments, "Conversion of the provided moments to parameters has failed!"
                ),
            )
        end
    ...
    if distribution == "Poisson"
        ...
        if !(mean ≈ (std ^ 2))
            throw(
                DomainError(
                    moments, "Standard deviation must be equal to square root of mean!"
                ),
            )
        end
