...
@testset "Sensitivity Problems (Type II) #1" begin
    ...
    Θ = [250, 250 * 0.30, 125, 125 * 0.30, 2257, 1 / 0.00257, 12.2, 41700]
    ...
    @test isapprox(
        solution.∇β,
        1E-3 * [-3.240, -3.920, -6.480, -7.840, -0.546, -1.334, +88.8, +0.0951],
        rtol=5E-2,
    )
    @test isapprox(
        solution.∇PoF,
        1E-4 * [+0.617, +0.746, +1.230, +1.490, +0.104, -0.254, -16.9, -0.0181],
        rtol=5E-2,
    )
end
...
@testset "Sensitivity Problems (Type II) #2" begin
    ...
    @test isapprox(
        solution.∇β,
        1E-3 * [-3.240, -3.920, -6.480, -7.840, -0.546, -0.790, 0.124, -0.245],
        rtol=5E-2,
    )
end
...
@testset "Sensitivity Problems (Type II) #3" begin
    ...
    # Test the results:
    @test isapprox(
        solution.∇β, [-0.0059, -0.0079, -0.0009, -0.0006, 1.2602, -1.1942], rtol=1E-2
    )
end
